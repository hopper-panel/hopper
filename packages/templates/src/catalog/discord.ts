...
const pythonInstallScript = [
  ...
  '# Your bot',
  '# ---------------------------------------------------------------------',
  '# Nothing is downloaded here. This template hosts code you put in the',
  '# volume yourself, through the file manager or SFTP, which means the very',
  '# first installation of a new server has nothing to install: the volume is',
  '# empty until you have uploaded something.',
  '#',
  '# That is not a failure and the script does not treat it as one. Upload the',
  '# bot, then press Reinstall — that is what installs the dependencies.',
  'cd /mnt/server',
  '',
  'if [ ! -f requirements.txt ]; then',
  '  echo "No requirements.txt in the volume, so nothing was installed."',
  '  echo ""',
  '  echo "Upload your bot - the .py files and a requirements.txt - through"',
  '  echo "the file manager or SFTP, then press Reinstall. That is what puts"',
  '  echo "the dependencies in place; the startup command cannot, because it"',
  '  echo "is a command and not a shell script."',
  '  exit 0',
  'fi',
  '',
  '# ---------------------------------------------------------------------',
...
const nodeInstallScript = [
  ...
  '# Your bot',
  '# ---------------------------------------------------------------------',
  '# Nothing is downloaded here: this template hosts code you upload. A brand',
  '# new server has an empty volume, so the first installation finds no',
  '# package.json and installs nothing. Upload the bot, then press Reinstall.',
  'cd /mnt/server',
  '',
  'if [ ! -f package.json ]; then',
  '  echo "No package.json in the volume, so nothing was installed."',
  '  echo ""',
  '  echo "Upload your bot through the file manager or SFTP, then press"',
  '  echo "Reinstall. That is what installs node_modules; the startup command"',
  '  echo "cannot, because it is a command and not a shell script."',
  '  exit 0',
  'fi',
  '',
  '# ---------------------------------------------------------------------',
